...
import { collection, doc, DocumentData, getDocs, query, QuerySnapshot, updateDoc, where } from 'firebase/firestore';
...
export async function getVerificationPassword(password: string) {
  ...
  try {
    ...
    // CORRECTION 1
    console.error('Erreur getVerificationPassword:', error);
    return false;
  }
...
export async function updateThePassword(password?: string) {
  ...
  try {
    ...
    // CORRECTION 2
    console.error('Erreur updateThePassword:', error);
...
export async function updateTheEmail(userId: string, email: string) {
  ...
  try {
    ...
    // CORRECTION 3
    console.error('Erreur updateTheEmail:', error);
    return 'La modification a échoué';
  }
...
export async function updateProfileUser(firstname: string, lastname: string, email: string, newPassword?: string) {
  ...
  try {
    ...
    // CORRECTION 4
    console.error('Erreur updateProfileUser:', error);
    return 'La modification a échoué';
  }
};