...
interface Task {
  ...
  Active: boolean;
  Status: 'completed' | 'pending' | 'overdue' | 'deleted';
  Tolerance: number;
  assignedMembers: string[];
  createdAt: FirebaseFirestoreTypes.Timestamp;
  deleted_at?: FirebaseFirestoreTypes.Timestamp | null;
  nextOccurrence?: Date;
  fixedTimes?: string[];
  startDateTime?: FirebaseFirestoreTypes.Timestamp | null;
}
...
const TaskManagerScreen: React.FC<TaskManagerScreenProps> = ({ navigation }) => {
  ...
  const { deleteTaskWithConfirmation } = useTaskDelete();

  // Récupération des tâches depuis Firestore
  useEffect(() => {
    const unsubscribe = TaskService.getActiveTasks().onSnapshot(
      (querySnapshot) => {
  ...
  const handleDeleteTask = (taskId: string, taskName: string) => {
    deleteTaskWithConfirmation(taskId, taskName);
  };
